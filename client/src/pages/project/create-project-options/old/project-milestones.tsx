...
const ProjectMilestones = ({ page, headers, changePage }) => {
  ...
  const [milestones, setMilestones] = useState<
    { milestone: string; target: number }[]
  >([])
  ...
  const handleNext = () => {
    if (milestones.length === 0) {
      toast.error('You must add at least one milestone before proceeding.')
      return
    }
  ...
  return (
    <>
      ...
      <Container className='w-75 mt-4'>
        ...
        <button className='btn-modal mt-5 px-0 mx-0' onClick={handleNext}>
          Proceed to Executors
        </button>
      </Container>
    </>
  )
